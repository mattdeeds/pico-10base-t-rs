...
## 👉 Next session — start here: R14.4 — the LAN DHCP server (`src/dhcp_server.rs`, a smoltcp UDP socket on :67). R14.1 (executor + continuous Runner), R14.2 (WPA2 AP), and R14.3 (phy adapter + LAN `Interface`, client pings `192.168.4.1` 5/5) are COMPLETE & on-device-validated. R14.4 is the last step of "LAN up" — then a client just *joins and gets an IP*. Scope stays NARROW & LAN-side only — NAPT stays R17. **Step plan: [`docs/router-plan.md`](docs/router-plan.md) §12.2.**
...
**▶ FIRST ACTION — R14.4: the LAN DHCP server (the last "LAN up" step).** New `src/dhcp_server.rs`: a smoltcp UDP socket bound to `0.0.0.0:67` in `net_task`'s `SocketSet`; parse BOOTP/DHCP DISCOVER + REQUEST; emit OFFER + ACK broadcast to `255.255.255.255:68` with yiaddr from a fixed `heapless` lease pool in `192.168.4.0/24`, router+DNS = `192.168.4.1`, subnet mask + lease time. Lease table keyed by client MAC. **Watch the DHCP broadcast quirks** (§12.3 risk 2): clients send from `0.0.0.0`→`255.255.255.255` before they hold an IP; the smoltcp UDP bind/broadcast-accept + the BOOTP broadcast flag are the fiddly part. **Accept (the R14 milestone):** a phone/laptop joins the SSID, **auto-gets a lease**, and pings `192.168.4.1` — no manual static IP. Then R14.5 = bind the existing HTTP server to `192.168.4.1:80` (mgmt page) + docs. **R14 stays a standalone-wireless build** (10BASE-T not started — §11); executor-⊥-10BT unification is deferred to R16. Validation client = the host's RTL88x2bu (`firmware-realtek`+`iw`+`wpasupplicant` now installed) — drop the static IP and let it DHCP. Full step plan: [`docs/router-plan.md`](docs/router-plan.md) §12.2.
