...
| **R6** — IRQ-driven RX | ✅ | RX state moved into a module-level `Mutex<RefCell<Option<EthRxShared>>>`; DMA channels `enable_irq0()`'d so each half-completion fires `DMA_IRQ_0`, whose handler runs the full stitch + decode + inbox-push pipeline. Main loop no longer polls — `iface.poll` drains the inbox via `Device::receive`. **2.18 ms main-loop budget is gone.** `EthTx::send_raw_frame` + `send_udp_broadcast` now wrap their PIO writes in `critical_section::with` so the IRQ can't preempt mid-frame and underrun the TX FIFO. Concurrent stress: UDP 100%, ping 97–99%, host RX errs 8 / 30 s (down from 20 without the TX critical section). |
| **Beyond R6** — pick from improvements list below | ⏳ | next |

Last verified: 2026-05-26 (post-R6, IRQ-driven RX with TX critical sections). 30-sec stress: ping 291–297/300 (97–99%), UDP echo 298–300/300 (99–100%). Telemetry as before — `dec=20 ok=20 fail=0 inbox_drop=0 inbox_hwm=1–2 carry_cap=0`. **The R5→R6 move is architecturally a clear win** (main-loop budget removed, decoder cadence decoupled from main work), but user-visible reliability is a wash: UDP improved by ~1 pp, ping regressed by ~1–2 pp, and host RX errs went from 0 → 8 / 30 s. The 8 residual errs are *not* explained by the obvious "IRQ preempts TX mid-frame" theory — that's covered by the `send_raw_frame` critical section, and the host-error count dropped from 20 to 8 when we added it. The remaining ~1 err/sec under load is the next thing to chase (see "Beyond R6" #1).
...
| `src/eth_rx.rs` | `EthRx` struct — PIO sampler, DMA double-buffer with **carry+stitch buffers** (R5), `poll_with` closure handoff over the stitched view, `find_active_run_from` (iterates all runs, not just longest) + `decode_frame` + `derive_frame_len` + `verify_fcs` |
| `src/eth_mac.rs` | `EthMac` — wraps `EthTx + EthRx` and implements `smoltcp::phy::Device`. Owns a TX scratch buffer + 4-slot RX `Deque` inbox (drop-oldest on overflow); pass-through `send_nlp` / `send_udp_broadcast`; `poll(cb)` walks every active run in the stitched buffer + populates inbox; `EthTxToken<'a>` + `EthRxToken` for smoltcp |
...
- **Residual FCS fails (~0–1/sec under load).** Rare CRC fails persist. `carry_cap=0` rules out cap-clipping, so the cause is elsewhere — likely some combination of: (a) genuine wire bit-errors, (b) phase-lock edge cases when the run starts on a noisy NLP, (c) the decoder's "longest run" → "find next run" change occasionally finding a spurious noise blob between frames. Worth instrumenting once it becomes the bottleneck on a real workload.
...
### Beyond R6 — improvements (priority order, pick whichever bites)

1. **Chase the residual ~1 host-RX-err / sec under load.** Pre-R6 had 0 host RX errs; R6 introduced the IRQ; the obvious TX preemption was covered by the `critical_section` in `send_raw_frame`. Still ~8 errs per 30 s of concurrent traffic. Hypotheses (test each cheaply): (a) NLP at 16 ms cadence colliding with frame trailing edges — wrap `send_nlp` in `critical_section` too and re-measure; (b) the host's NIC seeing a glitch during the post-frame TP_IDL → idle transition when the DMA IRQ fires immediately after `send_raw_frame` exits its critical section; (c) something specific to ICMP timing (UDP echo is at 100%, ICMP at ~97%, suggesting an asymmetry between smoltcp's auto-reply path and our explicit socket TX). A scope on the DI line during a 30-sec stress would settle it in 5 minutes.

2. **MAC filtering.** Currently we Manchester-decode + CRC every frame on the wire (including broadcasts we'll never act on). Skip frames whose dst MAC isn't ours, broadcast, or one of our multicast groups *before* the full decode — saves CPU per IRQ and shortens the critical_section'd decode window (which is what makes the TX-preemption story even worse than it has to be).

3. **TCP socket.** Add `socket-tcp` to the smoltcp feature list and bind a listener (e.g., a tiny HTTP server returning the per-second RX stats). Validates retransmission / windowing through our (now-much-better) RX path.
