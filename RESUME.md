...
## 👉 Next session — start here: R14.3 — the keystone `phy::Device` adapter over cyw43's `NetDriver` + the LAN smoltcp `Interface` (`192.168.4.1/24`). R14.1 (executor + continuous Runner) and R14.2 (WPA2 AP up, SSID seen on a phone) are COMPLETE & on-device-validated. Scope stays NARROW & LAN-side only — NAPT stays R17. **Step plan: [`docs/router-plan.md`](docs/router-plan.md) §12.2.**
...
**✅ R14.2 COMPLETE (2026-05-29) — WPA2 AP up (commit `c7ad6ae`).** The bootstrap task now does `set_power_management(None)` (an AP must stay awake) + `start_ap_wpa2("pico-rp2350-router", "picorouter2350", channel 6)` after `Control::init`. **On-device `[Cyw43] … ap=1` (hb climbing); SSID `pico-rp2350-router` confirmed visible on a phone.** Dev creds + channel in `wireless.rs` (`AP_SSID`/`AP_PASSPHRASE`/`AP_CHANNEL`). NB the `NetDriver` (`_net`) is still dropped — passive-scan only; *joining* needs R14.3+R14.4.

**▶ FIRST ACTION — R14.3: the `NetDriver` → smoltcp `phy::Device` adapter + the LAN `Interface`.** New `src/cyw43_phy.rs`: wrap cyw43's `NetDriver` (= `embassy_net_driver_channel::Device`, **NOT** a direct smoltcp plug) in a smoltcp `phy::Device` via its **synchronous** `try_rx_buf`/`rx_done(len)` + `try_tx_buf`/`tx_done()` API, using `borrow_split()` so the rx+tx tokens `receive()` returns borrow disjoint halves (no `embassy-net` dep, no noop-waker). Stop dropping `_net` in the bootstrap task; build a 2nd smoltcp `Interface` (static `192.168.4.1/24`, MAC from `Control::address()`), poll it in a net task. **Accept:** a *manually-configured* client (static `192.168.4.2/24`) pings `192.168.4.1` (auto-icmp-echo-reply is already in our smoltcp features) — proves the data path through cyw43 + the adapter before DHCP exists. Then R14.4 = the DHCP server (`src/dhcp_server.rs`, smoltcp UDP :67). **R14 stays a standalone-wireless build** (10BASE-T not started — §11); executor-⊥-10BT unification is deferred to R16. Full step plan + acceptance per step: [`docs/router-plan.md`](docs/router-plan.md) §12.2.
