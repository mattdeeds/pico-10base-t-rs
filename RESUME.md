...
## 👉 Next session — start here: multicore RX (Phase 3a/3c)

**Goal:** fix the **23× TCP throughput collapse under broadcast stress** (R11 measured 596 → 26 kB/s) by moving the RX IRQ off core 0 onto the idle 2nd Hazard3 core, so smoltcp + the main loop can't be starved by decode work. This is the only remaining blocker for the router-uplink goal in the `project-vision-router` memory.

**Design + first-step blocker already on disk:**
...
- **First-step blocker we hit and reverted last session: rp235x-hal 0.4's `multicore::Multicore::spawn` is gated to `target_arch = "arm"`** (uses Cortex-M VTOR/MSPLIM/ICB registers — won't compile or link for Hazard3 RISC-V). See §9a for what's missing: a custom `launch_core1_riscv` doing the SIO-FIFO bootstrap protocol `[0, 0, 1, vector_table, sp, entry]`, plus a small naked-asm trampoline that pops `(entry, stack_bottom, core1_wrapper, gp)` into a0/a1/a2/gp and `jr a2`s into our entry. The pico-sdk's RISC-V launch path is the cross-reference; the RP2350 datasheet §5.5.5 is the spec. Two hours of careful work, not a deep blocker.

**Suggested phasing (mirrors `docs/cpu-dpll-plan.md` §4):**

1. **Phase 3a — multicore foundation.** Solve the Hazard3 launch protocol from §9a. Bring up core 1 doing nothing useful (1 Hz "core1 alive: ticks=N" via the SIO FIFO). Main loop + USB + TX + RX unchanged on core 0. **Stress-test gate: existing R10 production behaviour preserved** (`cargo run --release` → ping 100%, UDP echo clean, 1 MB curl ~596 kB/s).
2. **Phase 3c — move `DMA_IRQ_0` to core 1.** Replace the `critical_section::Mutex` around `SHARED_RX` with `hal::sio::Spinlock<0>`. Route the IRQ via per-core NVIC enable so core 0 never sees it. Core 1 `wfi`s between IRQs. **Acceptance gate from R11: 1 MB curl under concurrent broadcast blast no longer collapses** (target: stays above ~300 kB/s, vs today's 26 kB/s).
...
| **R12** — multicore RX (Phase 3a/3c) | ⏳ **next session** | Move `DMA_IRQ_0` + the full RX decode pipeline (`process_completed_half`) from core 0 onto the idle 2nd Hazard3 core, so smoltcp + main can't be starved under broadcast stress. Architecture in [`docs/cpu-dpll-plan.md`](docs/cpu-dpll-plan.md) §3. **First obstacle (§9a, previously hit + reverted):** rp235x-hal 0.4's `multicore::spawn` is ARM-only; need a custom `launch_core1_riscv` + naked-asm trampoline. Phase 3a brings core 1 up doing nothing useful; Phase 3c routes the IRQ + swaps `SHARED_RX` from `critical_section::Mutex` to `hal::sio::Spinlock`. Acceptance: 1 MB curl under concurrent broadcast blast above ~300 kB/s (vs R11 baseline 26 kB/s, 23× collapse). See the "Next session — start here" callout at the top of this file for the suggested phasing and reference numbers. |
