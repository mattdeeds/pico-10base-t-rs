...
## 👉 Next session — start here: Phase 3c (move RX IRQ to core 1)

**Phase 3a (multicore foundation) is ✅ DONE + verified on-wire (2026-05-28).** The Hazard3 RISC-V core-1 launch blocker from §9a is solved — see `docs/cpu-dpll-plan.md` §9f for the write-up. Core 1 launches reliably (`[Core1] launch=ok ticks=N` climbing ~1000/s) while core 0's data path stays byte-identical; ping 20/20, curl 200 OK, UDP echo 10/10. Launch code: `src/multicore_riscv.rs`.

**Goal of 3c:** fix the **23× TCP throughput collapse under broadcast stress** (R11 measured 596 → 26 kB/s) by moving the RX IRQ off core 0 onto the now-running 2nd Hazard3 core, so smoltcp + the main loop can't be starved by decode work. This is the only remaining blocker for the router-uplink goal in the `project-vision-router` memory.

**Design already on disk:**

- Multicore architecture (cores split, shared inbox, IRQ routing): [`docs/cpu-dpll-plan.md`](docs/cpu-dpll-plan.md) §3.
- IPC choice (shared `heapless::Deque` + HW spinlock — option A): §5.
- Phase 3a launch solution + the four §9a fixes (mtvec not VTOR / gp trampoline / drop ACTLR / bounded reads): §9f.

**Phase 3c — move `DMA_IRQ_0` to core 1.** Replace the `critical_section::Mutex` around `SHARED_RX` with `hal::sio::Spinlock<0>`. Route the IRQ via per-core enable so core 0 never sees it (core 1 `wfi`s between IRQs, running `process_completed_half`). **Acceptance gate from R11: 1 MB curl under concurrent broadcast blast no longer collapses** (target: stays above ~300 kB/s, vs today's 26 kB/s — needs the `http-bulk-test` feature build to measure throughput).

⚠️ **Carrier-sense interaction (gotcha #10) is the thing to watch.** Moving the IRQ to core 1 *removes the accidental carrier-sense* that today's single-core design relies on: core 0 currently can't TX while it's busy decoding (same core); after 3c, core 0 is free to TX while core 1 decodes, so it can transmit into an in-flight host frame under load. The 30-s broadcast-blast curl measurement is the decision point for whether CSMA/CD becomes the next priority (interim: core 0 checks a shared "RX-active" atomic before TX). This was discussed + chosen as "multicore-first, measure at 3c" — don't speculatively build CSMA/CD first.

**Reference points to compare against (R11 baseline):**
- Production DPLL @ 240 MHz @ 60 MHz sample, **idle wire**: TCP 596 kB/s, ping 100%, FCS-OK ~100% on the small frames it actually sees.
- Same firmware, **30 s broadcast blast @ 50 pps + ping flood**: TCP 26 kB/s (first curl), FCS-OK 70.7% on full-MTU broadcasts, ping 89.7%. Anything Phase 3c does has to leave the idle-wire numbers alone and push the stressed numbers up.

**Out of scope for this branch** — same as the earlier multicore plan: NAT/forwarding, full-duplex, PIO CSMA, the wireless interface. Just the RX IRQ relocation.
...
| **R12c** — move RX IRQ to core 1 (Phase 3c) | ⏳ **next** | Route `DMA_IRQ_0` + the full RX decode pipeline (`process_completed_half`) onto the now-running core 1, so smoltcp + main can't be starved under broadcast stress. Swap `SHARED_RX` from `critical_section::Mutex` to `hal::sio::Spinlock`; core 1 `wfi`s between IRQs. Acceptance: 1 MB curl under concurrent broadcast blast above ~300 kB/s (vs R11 baseline 26 kB/s, 23× collapse) — needs the `http-bulk-test` build. ⚠️ Moving the IRQ off core 0 removes the accidental carrier-sense (gotcha #10): the broadcast-blast curl is the decision point for whether CSMA/CD is the next priority. See the "Next session — start here" callout for details. |
