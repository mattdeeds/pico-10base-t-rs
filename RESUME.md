...
## 👉 Next session — start here: R15b — runtime unification (both interfaces live). **R15a is COMPLETE & on-device-validated (2026-05-29): the 10BASE-T side is now a DHCP *client* — it leased `192.168.37.129`, pinged `8.8.8.8` (echo replies returning, id 0x42), and resolved `example.com` out the wired link.** R15 was split into two sub-steps (see [`docs/r15-plan.md`](docs/r15-plan.md) §2, which reconciles RESUME-vs-§12): **R15a** = DHCP client on the standalone 10BT build (DONE, de-risked the WAN-reachability piece in isolation, no executor); **R15b** = the **executor-⊥-10BT runtime unification** (§11 deferral) — run BOTH interfaces: 10BT RX IRQ on core 1 + the new DHCP-client WAN logic as an async task while the embassy executor owns core 0 and drives the cyw43 LAN. NAPT still stays R17. **Step plan: [`docs/r15-plan.md`](docs/r15-plan.md) §6 (R15b) + the reframe in §3 (it's mostly wiring — the executor + 2nd-Interface-as-task + core-1-RX are each already proven).**

**⚠️ The board currently runs the `--features wan-dhcp` image** (flashed 2026-05-29 for R15a validation — a 10BT build, NOT wireless). To restore the 10BT production build: `cargo run --release` (default, static IP `192.168.37.24`). For wireless LAN: `cargo run --release --features wireless`. For the R15a WAN-client build: `cargo run --release --features wan-dhcp`.

**▶ R15a test harness: [`tools/wan-test-host.sh`](tools/wan-test-host.sh)** — turns this host into the device's upstream gateway (dnsmasq DHCP+DNS on `enp1s0f0` + nftables masquerade out the auto-detected uplink `eno1`; reverts on Ctrl-C). Host now has `nftables` (Debian 13 native — NOT iptables) + `dnsmasq-base` installed. `sudo tools/wan-test-host.sh`, then flash `--features wan-dhcp`, then `tcpdump -ni eno1 icmp and host 8.8.8.8` (or read the device `[Wan]` CDC line) to confirm the round-trip.
...
**✅ R15a COMPLETE (2026-05-29) — WAN as a DHCP client (standalone 10BT build, branch `r13-wireless-scaffold`, uncommitted).** New `wan-dhcp` cargo feature (OFF by default — the production static-IP build is byte-unchanged) adds three smoltcp sockets to `main_10bt` (`socket-dhcpv4`/`-icmp`/`-dns`): the **dhcpv4 client** drops the static `192.168.37.24` and installs the leased address + default route + DNS on `Configured`; an **icmp socket** pings `8.8.8.8` (ident `0x42`, `"pico-wan"` payload) once/sec; a **dns socket** resolves `example.com` (A). New `[Wan]` CDC line: `ip=… gw=… dns=… ping=ok/sent <name>=<A>`. Helper `tools/wan-test-host.sh` makes the host the upstream gateway (dnsmasq + nftables masquerade). **On-device: dnsmasq logged `DHCPACK 192.168.37.129` to the device MAC + forwarded `example.com`; `tcpdump -ni eno1` showed `192.168.0.33 > 8.8.8.8 ICMP echo request id 66 len 16` with matching replies ~13 ms, once/sec — all three acceptance criteria (lease + ping + resolve) met.** This de-risked the WAN-reachability piece in isolation (no executor). All in `src/main.rs` (gated helpers `wan_dhcp_apply`/`wan_ping_send`/`wan_ping_drain`/`wan_dns_start`/`wan_dns_harvest`/`log_wan`).

**▶ FIRST ACTION — R15b: runtime unification (both interfaces live).** Now run the R15a WAN-client logic AND the cyw43 LAN concurrently. New `router` cargo feature = `wireless` + the WAN sockets; `main()` gets a third dispatch arm that does the 10BT bring-up (PIO0 + DMA + `install_rx` + launch core 1, **no GP25 LED** — GP25 is the gSPI CS) + the PIO1 gSPI + WL_ON, then hands `EthMac` to a `wireless::run_router` that adds a `wan_task(EthMac)` (the R15a logic, async, NLP @16 ms) beside the existing `cyw43_bootstrap_task`/`net_task`. **Reframe (docs/r15-plan.md §3):** this is mostly *wiring* — the executor (R14.1), a 2nd `Interface`-as-task (R14.3 `net_task`), and core-1 RX (R12c) are each already proven; the new surface is narrow (N1–N4 in the plan). **Accept (concurrent):** WAN out (R15a criteria under the executor) + LAN intact (re-run R14) + core 1 up. **Step plan:** [`docs/r15-plan.md`](docs/r15-plan.md) §6 + the §9 checklist.
