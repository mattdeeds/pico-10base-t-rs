...
## 👉 Next session — start here: R14 — the router proper (cyw43 `NetDriver` → smoltcp LAN + AP + DHCP + NAPT). R13 (wireless bring-up) is COMPLETE.
...
**▶ FIRST ACTION — R14, the router data path** (`docs/router-plan.md` §6/§7). This is where the **persistent embassy executor + a continuous `Runner`** finally earn their keep — cyw43 must stay up for networking, so the R13 `block_on` bring-up is throwaway scaffolding. Steps: wrap the cyw43 `NetDriver` in a smoltcp LAN `Interface`; AP mode (`Control::start_ap_wpa2`) + a DHCP server (smoltcp has only a client); then the forwarding/NAPT data path between the WiFi LAN and the 10BASE-T WAN. **The #1 deferred risk lands here: the async runtime on Hazard3 at *runtime* + driving telemetry/USB from the executor** (R13 sidestepped it with `block_on`; R14 can't). Likely needs the standalone-wireless-build fork (executor owns a core).
