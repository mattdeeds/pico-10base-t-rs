...
1. **MAC filtering.** Currently we Manchester-decode + CRC every frame on the wire (including broadcasts we'll never act on). Skip frames whose dst MAC isn't ours, broadcast, or one of our multicast groups *before* the full decode — saves CPU per IRQ and shortens the critical_section'd decode window (relevant for any future workload pushing more than ~20 frames/sec).

2. **TCP socket.** Add `socket-tcp` to the smoltcp feature list and bind a listener (e.g., a tiny HTTP server returning the per-second RX stats). Validates retransmission / windowing through our RX path under more demanding traffic patterns than UDP echo.
