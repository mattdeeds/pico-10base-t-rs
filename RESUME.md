...
## 👉 Next session — start here: R13 Steps 2–5 (async cyw43 bring-up) — Step 1 PIO gSPI transport is PROVEN
...
- ✅ **Board wireless VERIFIED GOOD (2026-05-28)** with stock MicroPython v1.28.0 (`RPI_PICO2_W` ARM UF2), driven over the REPL with `mpremote`: `Pin('LED')` on/toggle OK (→ CYW43 chip + gSPI bus alive) and `network.WLAN(STA_IF).scan()` → **12 APs** (RSSI −29..−83), CYW43 MAC **`2c:cf:67:df:78:2f`** (Raspberry Pi OUI). Radio + firmware-download over gSPI both work. **⇒ The hardware is fine — our `wireless::probe_cyw43` bit-bang gSPI code is the bug** (it had read varying/floating data; pin map GP23=WL_ON/24=DATA/25=CS/29=CLK + funcsel=5 were ruled out, SPI lines off-header/unscoppable). Our firmware restored after (`picotool load -x -t elf …` → re-enumerated `2e8a:000a`). **Don't re-flash MicroPython to re-verify.**

**✅ R13 STEP 1 DONE — PIO1 gSPI transport PROVEN.** `wireless::probe_cyw43_pio` reads **`0xFEEDBEAD`** from the CYW43 bus test register over a self-contained FIFO-driven PIO1 gSPI SM (gated by `--features wireless`; result on the `[Cyw43]` CDC telemetry line). Fixes vs the dead bit-bang: align the PIO program to embassy `cyw43-pio` 0.7.0 (sample DATA on CLK-high, `nop side 0` turnaround, read data+status word) **and hold the gSPI bus idle (CLK low, CS high, DATA low) through the WL_ON power-up** — see gotcha #11. Pin map confirmed against pico-sdk `pico2_w.h` (23/24/25/29). (The bit-bang `probe_cyw43` is kept for reference; the self-test `pin_selftest` confirmed the pads drive.)

**▶ FIRST ACTION — R13 Steps 2–5** (full plan: `docs/router-plan.md` §11): (2) move the proven FIFO push/pull into the async `SpiBusCyw43::cmd_write/cmd_read` (busy-poll for now); (3) `cyw43::new` + 3 aligned blobs — fw + **nvram** + clm (nvram sourcing for the 0.7.0 API is the top unknown); (4) CDC-in-async telemetry (log ring + a usbd-serial executor task, spawned *before* bring-up); (5) onboard-LED blink + `[Cyw43] init done` ⇒ R13 met. NB: this forks to a **standalone wireless-only build** (the embassy executor owns core 0; 10BASE-T deferred to R14).
