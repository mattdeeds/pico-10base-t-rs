...
## 👉 Next session — start here: R14 — "LAN up" (cyw43 AP + smoltcp-on-`NetDriver` + DHCP server). Scope is NARROW & LAN-side only — NAPT stays R17. R13 (wireless bring-up) is COMPLETE. **Step plan: [`docs/router-plan.md`](docs/router-plan.md) §12.**
...
**▶ FIRST ACTION — R14.1 (the core de-risk): graduate the cyw43 bring-up from `block_on` to the persistent embassy executor with a continuous `Runner`** — see the full step plan in [`docs/router-plan.md`](docs/router-plan.md) §12. R13's `block_on(select(runner, seq))` *returned* after 6 LED blinks; R14 needs `Runner::run()` alive forever (it services every gSPI transaction), so R14.1 spawns it as a long-lived executor task and the LED must blink *indefinitely*. **This is where the #1 deferred risk lands — the async runtime on Hazard3 at *runtime* (the executor compiles + links since R13 but has never been *run*).** Decisions locked (§12.1): (1) **persistent executor** (fallback: `block_on(join3(...))`); (2) the cyw43 `NetDriver` is `embassy_net_driver_channel::Device` — **NOT** a direct smoltcp plug; R14.3 writes a thin `phy::Device` adapter over its sync `try_rx_buf`/`try_tx_buf`/`borrow_split` API (no `embassy-net` dep). Then R14.2 AP (`start_ap_wpa2`), R14.3 LAN `Interface` (static `192.168.4.1/24`), R14.4 DHCP server. **R14 stays a standalone-wireless build** (10BASE-T not started — §11); executor-⊥-10BT unification is deferred to R16. Also: **relocate the heartbeat LED off GP25** (= WL_CS on Pico 2 W; `main.rs:163`) onto `Control::gpio_set(0,…)`.
