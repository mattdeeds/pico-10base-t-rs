...
2. ~~**Stitch scan-in-place — priority #2.**~~ **DONE (2026-05-27).** The 16 KB `copy_from_slice` ran every half (~296 µs of the IRQ).
   ...
   - **Validation (on wire):** stress ×3 ping 99.7–100 % / UDP echo 99.7–100 % / host RX errs 0–2 per 30 s (best of the session — at/above every prior baseline); multi-size echo 20/20 byte-perfect at 96/199/346/646 B (straddle handling correct — UDP echoes cross boundaries under load). Clippy clean. Copy-elimination rate measured separately — see below.
