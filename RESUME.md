...
## 👉 Next session — start here: Phase 3d (carrier-sense / CSMA)

**The multicore measurement is in, and it's decisive: carrier-sense — not core separation — is the real lever for TCP throughput.** Phases 3a + 3c are both done and on-wire-measured (2026-05-28):

- **Phase 3a (multicore foundation): ✅ DONE + merged to `main`.** Hazard3 RISC-V core-1 launch works (`src/multicore_riscv.rs`); §9a blocker solved. Write-up §9f.
- **Phase 3c (RX decode on core 1): ✅ DONE, on branch `r12c-multicore-rx`, NOT merged.** It *fixes* the A1/R11 CPU starvation (under a 50 pps full-MTU blast, core 1 decodes the full rate while core 0 stays at full cadence) — **but** removing the accidental carrier-sense (gotcha #10) collapses idle 1 MB curl 596 → ~45 kB/s (~12×) from Pico-TX-vs-host-ACK collisions (~30 host collisions/curl, confirmed via `/proc/net/dev`). So 3c is a net throughput **regression** until carrier-sense lands. Full write-up + the precise reframing: [`docs/cpu-dpll-plan.md`](docs/cpu-dpll-plan.md) §9g.

**Goal of 3d:** restore the carrier-sense Phase 3c removed, so core 0 stops transmitting into in-flight host frames. With that, the multicore RX becomes a clear net win (no starvation *and* no collisions) and `r12c-multicore-rx` can merge.

**Options (rough order of fidelity — see §9g):**

1. **Real PIO carrier-sense in the TX path (the right fix).** Before the TX SM starts a frame, sense RO (GP13) for carrier and defer if busy; ideally collision-detect for CSMA/CD backoff. Substantial PIO work but it's proper half-duplex MAC behaviour. The TX PIO program lives in `src/eth_tx.rs`.
2. **Software carrier-sense interim.** Core 0 checks a cheap "wire busy" signal (derived from recent RX-sampler activity) before TX. Imperfect (DMA-half ~2.18 ms granularity lags real wire state) but a fast way to validate the diagnosis + recover much of the throughput.
3. **smoltcp tuning band-aid** (bigger TX window → fast-retransmit instead of RTO). Symptom-only, fragile, not recommended.

**Mind the trade-off:** carrier-sense reintroduces TX↔RX coupling. Real PIO carrier-sense keeps it at the wire level (cheap, correct). A software check that makes core 0 wait on core 1 risks partially re-coupling the cores — keep the wait tied to actual wire state, not to "core 1 is decoding."

**Reference points (R11 single-core baseline + Phase 3c measured):**
- Single-core, **idle wire**: TCP **596 kB/s**, ping 100%. (Accidental carrier-sense ⇒ collision-free.)
- Single-core, **30 s blast @ 50 pps + ping flood**: TCP 26 kB/s, FCS-OK 70.7%, ping 89.7% (starvation + collisions).
- **Phase 3c (branch), idle wire**: TCP **~45 kB/s** (collisions), ping 100% @ 2.3 ms.
- **Phase 3c (branch), pure-RX blast**: core 1 `ok≈50/s` (full rate), core 0 `nlps=62–63/s` steady — starvation gone.
- 3d target: idle TCP back to ~596 kB/s (collision-free) *and* blast TCP ≥ 300 kB/s.

**Working state to restore:** `main` = single-core 596 kB/s baseline. For 3d work put the multicore RX back on the device: `git checkout r12c-multicore-rx && cargo run --release` (add `--features http-bulk-test` to measure throughput).

**Out of scope** — NAT/forwarding, full-duplex, the wireless interface. Carrier-sense (3d) is now *in* scope (Phase 3c made it the blocker).
...
| **R12d** — carrier-sense / CSMA (Phase 3d) | ⏳ **next** | Add carrier-sense so core 0 doesn't TX into in-flight host frames (the accidental carrier-sense Phase 3c removed). Real fix: PIO carrier-sense in the TX path (sense RO/GP13 before starting a frame, ideally collision-detect for CSMA/CD backoff). Interim: software wire-busy check before TX. Once throughput recovers, merge `r12c-multicore-rx` → the multicore RX becomes a clear net win. See §9g for the options + the "Next session" callout. |
