...
## 👉 Next session — start here: R14.2 — start the AP (`Control::start_ap_wpa2`). R14.1 (persistent executor + continuous Runner) is COMPLETE & on-device-validated. Scope stays NARROW & LAN-side only — NAPT stays R17. **Step plan: [`docs/router-plan.md`](docs/router-plan.md) §12.2.**
...
**▶ FIRST ACTION — R14.2: bring up the AP.** After init, call `Control::start_ap_wpa2(SSID, passphrase, channel)` (sig confirmed, cyw43 0.7.0) in the bootstrap task; read the MAC via `Control::address()`. **Accept:** the SSID appears in a phone/laptop wifi scan (no IP yet — DHCP is R14.4). Then R14.3 = the keystone **`phy::Device` adapter** over cyw43's `NetDriver` (= `embassy_net_driver_channel::Device`, **NOT** a direct smoltcp plug — use its sync `try_rx_buf`/`rx_done`/`try_tx_buf`/`tx_done` + `borrow_split`; no `embassy-net` dep) + a 2nd smoltcp `Interface` (static `192.168.4.1/24`); R14.4 = the DHCP server. **R14 stays a standalone-wireless build** (10BASE-T not started — §11); executor-⊥-10BT unification is deferred to R16. Full step plan + acceptance per step: [`docs/router-plan.md`](docs/router-plan.md) §12.2.
