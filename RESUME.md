...
## 👉 Next session — start here: R14.5 — mgmt sanity + docs (bind the existing tiny HTTP server to `192.168.4.1:80` so a joined client loads a status page), then R14 "LAN up" is DONE → on to R15 (WAN as DHCP client). **R14.1–R14.4 are all COMPLETE & on-device-validated: a client joins the WPA2 AP, auto-gets a DHCP lease (`192.168.4.10`), and reaches the gateway.** Scope stays NARROW & LAN-side only — NAPT stays R17. **Step plan: [`docs/router-plan.md`](docs/router-plan.md) §12.2.**
...
**▶ FIRST ACTION — R14.5: mgmt sanity + docs (last R14 step, then R15).** Bind the existing tiny HTTP server (`serve_http` in `main.rs`) to the LAN `Interface` (`192.168.4.1:80`) — add a TCP socket to `net_task`'s `SocketSet` + a small serve loop — so a joined client loads a status page (uptime, lease count, AP info). Reuses the R8 server shape. **Accept:** a joined client browses to `http://192.168.4.1/`. Then R14 is complete → **R15 = WAN as DHCP client** (smoltcp `dhcpv4` socket on the 10BASE-T side → upstream IP/route/DNS), which also forces the executor-⊥-10BT runtime unification deferred from R14 (§11) — the 10BT RX IRQ on core 1 while the executor owns core 0. **R14 stays a standalone-wireless build.** Full step plan: [`docs/router-plan.md`](docs/router-plan.md) §12.2.
