...
| **R14.5 — mgmt sanity + docs** ✅ DONE (`a745875`) | `net_task` gains a TCP `:80` socket + `serve_status_http` (one-shot HTTP/1.0) showing AP SSID, LAN gateway, uptime, DHCP/rx counters. | ✅ joined client `curl http://192.168.4.1/` returned the status page (uptime 124 s, LAN rx 5). **R14 "LAN up" COMPLETE.** |
